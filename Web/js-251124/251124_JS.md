...
=> **이전 주문이 처리되는 동안 다음 주문을 처리할 수 있음**
...
- 따라서 '작업 2 시작'이 먼저 출력되고, 3초가 지난 후에 예약되었던 **콜백 함수**가 실행
...
- JavaScript Engine의 **Call Stack** -> 코드 실행하는 애 
  - 코드가 실행되면 함수 호출이 순서대로 쌓이는 작업 공간
  - 기본적인 JavaScript의 Single Thread 작업 처리 

- **Web API** -> 비동기 함수 처리하는 애 
...
비동기 함수의 작업이 끝나면, Task Queue로 보내서
콜백 함수들이 순서대로 작업이 처리될 수 있도록 함
...
Event Loop가 콜스택이 비어있는지 계속해서 확인

### B.

Call Stack이 비어있는 것을 확인한 후, Task Queue에서
가장 오래된 작업을 Call Stack으로 보냄
...
② 함수가 Call Stack으로 이동한 후, 함수 내부가 동작
...
* XML 
- 직접 태그를 만들어 데이터를 구조화하는 마크업 언어
* XMLHttpRequest
- 페이지 새로고침 없이 서버와 데이터를 주고받게 해주는 기술
...
- 웹 브라우저와 서버 간의 비동기 통신을 가능하게 하는 JacaScript 객체 
...
### 기존 기술과의 차이(2) - Ajax
1. XHR 객체 생성 및 요청
2. 서버는 새로운 페이지를 응답으로 만들지 않고 필요한 부분에 대해 데이터만 처리 후 응답(JSON 및 기타 데이터)
...
- 데이터 변환이나 에러 처리 등이 편리해, 많은 실무 프로젝트에서 HTTP 통신을 위해 가장 인기 있게 사용됨
...
- **XHR 객체 생성 및 요청** -> **응답 데이터 생성** -> **JSON 데이터 응답** -> **Promise 객체를 활용해 DOM 조작**(웹 페이지의 일부분 만을 다시 로딩)
...
- **성공 처리**
  - 서버로부터 받은 응답 데이터를 처리
  - **then 메서드**를 사용해서 “성공했을 때 수행할 로직” 작성
  - then(callback)
    - 요청한 작업이 성공하면 callback 실행
    - callback은 이전 작업의 성공 결과를 인자로 전달받음

- **실패 처리**
  - 네트워크 오류, 서버 오류 등 예외 상황을 처리
  - **catch 메서드**를 사용해서 “실패했을 때 수행할 로직” 작성
  - catch(callback)
    - then()이 하나라도 실패하면 callback 실행 (남은 then은 중단됨)
...
### 고양이 사진 가져오기 실습 심화 - (1/5)
1. 버튼을 누르면
2. 고양이 이미지를 요청하고
3. 요청이 처리되어 응답이 오면
4. 응답 데이터에 있는 이미지 주소 값을 img 태그에 넣어 이미지를 출력 


### 고양이 사진 가져오기 실습 심화 - (2/5)
...
- 개발자 입장에서 **코드의 실행 순서가 불명확**하다는 단점 존재하고, 이로 인해 실행 결과를 예측하며 코드를 작성하기 어려울 수 있음
...
- 이 과정을 작성하다 보면 비슷한 패턴이 계속 발생
  ...
  두 번재 callback 함수가 종료되면, 세번 째 callback 함수를 실행하고 ..
...
- 이런 문제를 해겨하기 위해 다른 코드 작성 방식이 필요
...
- then 메서드를 통해 여러 비동기 작업을 연결하는
  체이닝(Chaining) 하여 순차적으로 연결
...
### then & catch의 chainig
- axios로 처리한 비동기 로직은 항상 promise 객체랄 반환
...
### then 메서드 chaining의 장정

1. 가독성
  - 비동기 작업의 순서와 이존 관계를 명확히 표현할 수 있어 코드의 가동석이 향상
...
1. 실행 순서의 보장
  - 콜백 함수: JavaScript의 Event Loop가 실행 중인 Call Stack을 완료하기 전에는 호출되지 않음
...
4. 에러 처리의 일원화
  ...
  - 전통적인 콜백 방식에서 각 콜백마다 에러 처리를 해야 하는 번거러움을 해소
...
1. 동기식 처리의 한계 
  - 큰 데이터를 불어오는 작업을 동기식으로 처리할 경우, 데이터 로딩이 완료될 때까지 전체 앱의 실행이 차단
...
2. 비동기 처리의 장점
  ...
  - 데이터나 작업 결과가 준비되는 대로 순차적으로 화면에 표시할 수 있어, 사용자에게 진행 사항을 보여줄 수 있음 
...
* 비동기 처리에 대한 디테일한 개발은 사용자 경험을 많이 향상 시킬 수 있음
  - 요청 시작 즉시 로딩 스피너를 보여주면, 사용자가 시스템이 멈췄다는 오해를 막을 수 있음
  - 요청 실패 시, '다시 시도'와 같은 행동르 안내해야 함
...
- 예시) 비동기 결제 처리
  ...
  3. (동기) 응답이 '성공'일 경우, 우리 서버 데이터베이스에 주문 상태에 '결제 완료'로 기록
