<div align="center">

![TIL header](https://capsule-render.vercel.app/api?type=waving&color=0:FFDEE9,100:B5FFFC&height=190&section=header&text=Today%20I%20Learned&fontSize=42&fontColor=343434&fontAlignY=38&animation=fadeIn)

[![Naver Blog](https://img.shields.io/badge/Naver%20Blog-03C75A?style=for-the-badge&logo=naver&logoColor=white)](https://blog.naver.com/seol1n_)
![TIL](https://img.shields.io/badge/TIL-growing-FFD6E7?style=for-the-badge)
![Updated](https://img.shields.io/badge/updated-2026.07.28-B5FFFC?style=for-the-badge)

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=2600&pause=900&color=FF78A8&center=true&vCenter=true&width=620&lines=Small+notes,+steady+growth.;Learning+today+for+future+me.;Keep+it+clear,+keep+it+kind." alt="Typing SVG" />

<br/>

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=222222)
![Vue](https://img.shields.io/badge/Vue-4FC08D?style=flat-square&logo=vuedotjs&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=222222)
![Django](https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

</div>

---

## About

배운 내용을 그냥 흘려보내지 않기 위한 개인 학습 기록 저장소입니다.

개념을 처음 배운 날의 언어로 정리하고, 헷갈렸던 부분은 다시 봤을 때 바로 떠올릴 수 있게 예제와 풀이 흐름을 함께 남깁니다. 알고리즘 문제는 복습하기 쉽도록 문제 이름이 아니라 알고리즘 유형별로 정리하고 있습니다.

## Categories

| Category | Notes |
| --- | --- |
| [Algorithm](./Algorithm) | 개념 노트, 실습 코드, 유형별 문제 풀이 |
| [Python](./Python) | 문법, 자료구조, OOP, 예외 처리, 데이터 사이언스 기초 |
| [Java](./Java) | Java 문법, 배열, 문자열, 객체지향 기초 |
| [Web](./Web) | HTML, CSS, Bootstrap, JavaScript, 반응형 레이아웃 |
| [Django](./Django) | Django 구조, MTV 패턴, ORM, REST API |
| [Vue](./Vue) | Vue 3, Composition API, Router, Pinia, DRF 연동 |
| [React](./React) | React 기초와 Vite 기반 실습 프로젝트 |
| [Sql](./Sql) | SQL 문법, 쿼리, 모델링, 데이터베이스 기초 |
| [Git](./Git) | Git 명령어와 협업 흐름 |
| [Spring](./Spring) | Spring 학습을 위한 자리 |

## Algorithm Map

`Algorithm/problem`은 문제 풀이를 알고리즘 유형별 폴더로 나눠 정리했습니다.

<details>
<summary><b>유형별 문제 풀이 현황 보기</b></summary>

| Type | Count |
| --- | ---: |
| [Implementation / Simulation](./Algorithm/problem/01_Implementation_Simulation) | 56 |
| [Brute Force](./Algorithm/problem/02_BruteForce) | 11 |
| [Backtracking](./Algorithm/problem/03_Backtracking) | 41 |
| [DFS / BFS](./Algorithm/problem/04_DFS_BFS) | 26 |
| [Dijkstra](./Algorithm/problem/05_Dijkstra) | 7 |
| [0-1 BFS](./Algorithm/problem/06_0-1_BFS) | 1 |
| [Floyd-Warshall](./Algorithm/problem/07_Floyd_Warshall) | 1 |
| [MST](./Algorithm/problem/08_MST) | 3 |
| [Union Find](./Algorithm/problem/09_UnionFind) | 2 |
| [DP](./Algorithm/problem/10_DP) | 12 |
| [Greedy](./Algorithm/problem/11_Greedy) | 8 |
| [Sort](./Algorithm/problem/12_Sort) | 7 |
| [Binary Search](./Algorithm/problem/13_BinarySearch) | 5 |
| [Two Pointer / Sliding Window](./Algorithm/problem/14_TwoPointer_SlidingWindow) | 3 |
| [Stack](./Algorithm/problem/15_Stack) | 14 |
| [Queue / Deque](./Algorithm/problem/16_Queue_Deque) | 10 |
| [Heap / Priority Queue](./Algorithm/problem/17_Heap_PriorityQueue) | 5 |
| [Segment Tree](./Algorithm/problem/18_SegmentTree) | 1 |
| [Tree](./Algorithm/problem/19_Tree) | 8 |
| [String](./Algorithm/problem/20_String) | 6 |
| [Hash](./Algorithm/problem/21_Hash) | 5 |
| [Bitmask](./Algorithm/problem/22_Bitmask) | 7 |
| [Math](./Algorithm/problem/23_Math) | 8 |
| [Prefix Sum](./Algorithm/problem/24_PrefixSum) | 1 |
| [Divide Conquer / Meet in the Middle](./Algorithm/problem/25_DivideConquer_MeetInTheMiddle) | 2 |
| [Mo Algorithm](./Algorithm/problem/26_Mo_Algorithm) | 1 |

</details>

## My Rules

- 배운 내용은 나중의 내가 다시 읽어도 이해할 수 있게 정리하기
- 문제 풀이는 코드만 두지 말고 핵심 아이디어와 실수 포인트를 남기기
- 오래된 내용은 현재 폴더 구조와 학습 흐름에 맞춰 계속 다듬기
- 미완성 코드와 연습 조각은 정리하거나 과감히 비우기

---

<div align="center">

![footer](https://capsule-render.vercel.app/api?type=waving&color=0:B5FFFC,100:FFDEE9&height=110&section=footer)

</div>
