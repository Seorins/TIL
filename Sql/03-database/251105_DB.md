...
- 관계를 표현하기 위해, 한 테이블에 다른 테이블의 기본 키(Primary Key)를 저장하며, 이처럼 다른 테이블의 기본 키를 참조하는 속성을 외래 키(Foreign Key)라 함
...
- SET_NULL 
  ...
  - 단, 해당 ForeignKey 필드 설정이 null = True 설정 되어야 함 
...
* 현재 어떤 게시글에 작성되는 댓글인지 저장될 댓글 데이터에 정보가 없는 상태 
...
- 직접 객체를 할당하는 경우는 객체 타입을 에러 발생 유무를 통해 명확히 검증할 수 있음
- 직접 pk를 입력하는 경우에는 숫자를 저장하기 때문에 잘못된 객체의 PK값이 저장될 수 있음
...
### 댓글 생성 연습(4)
```bash
commentary = Comment(content='second content', article=article)
commentary.save()
...
- 댓글(Comment)에는 게시글(Article) 정보를 저장하는 ForeignKey 필드인 article이 존재하며, 해당 필드를 통해 게시글 정보에 쉽게 접근 할 수 있음
...
- 따라서 해당 글을 참조하고 있는 댓글들을 역으로 찾아야 하며 Django는 역참조 기능을 제공 


### 역참조 기본 구조 

`article.commet_set.all()`
...
- 모델 클래스명 + _set이 기본 값이며 Django에서 자동으로 생성해 줌
...
- 사용자로부터 댓글 데이터를 입력 빋기 위한 CommentForm 정의
...
* fields 설정을 tuple로 진행시 요소가 하나일 때, 마지막 콤마(,)를 반드시 작성해야 함 


### 댓글 CREATE(6)
- 출력에서 제외된 외래 키 데이터는 어디서 받아야 할까?
- detail 페이지의 URL에 게시글 정보가 존재
  - path('<int:pk>/', views.detail, name='detail')
  - 해당 게시글의 pk값이 variable routing으로 전달되고 있음
- 댓글의 외래 키 데이터에 필요한 정보가 바로 게시글 pk 값
...
```html
<!-- articles/detail.html -->
<form action="{% url 'artilces:comments_create' article.pk %}" method="POST">
  {% csrf_token %}
  {{ comment_form }}
  <input type="submit">
</form>
...
- commit이 False인 경우 DB에 저장 요청을 보내지 않고 인스턴스만 반환
  - Create, but don't save the new instace
...
def comments_create(request, pk):
  ...
  if comment_form.is_valid():
    ...
    comment_form.save() # 댓글 정보 DB에 저장 요청
    return redirect('articles:detail', article.pk)
  # 유효성 검사에 실패 했을 때 에러 정보 및 필요 데이터 전달 
  context = {
    'article:article, 
...
def detail(request, pk):
  article = Article.objects.get(pk=pk)
  comment_form = CommentForm()
  # 특정 게시글의 댓글이 아닌 모든 댓글을 조회하기에 사용 하지 않음
...
urlpatters = [
  ...,
  path(
    '<int:article_pk>/comments/<int:comments_pk>/delete/',
...
* article_pk 정보는 URL 패턴을 일관되게 작성하기 위함 외에도 삭제 후 detail 페이지로 돌아갈 때 사용할 수 있는 정보이기 때문에 작성함
